...
export default function UploadForm() {
  ...
  const upload = async () => {
    ...
    try {
      ...
      const { analysisId, warning } = res.data;

      if (warning) {
        setStatus(warning + " — analysis may be incomplete.");
      } else {
        router.push(`/analysis/${analysisId}`);
      }
    } catch (err: any) {
      console.error(err);
      setStatus(err.response?.data || "Upload failed. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="w-full min-h-screen flex items-center justify-center bg-midnightblue px-4">
      <div className="w-full max-w-xl p-8 rounded-3xl bg-white/5 border border-white/10 backdrop-blur-xl shadow-2xl flex flex-col gap-6 items-center">
        ...
        <p className="text-slate-300 text-center">PDF only — recruiter-ready AI analysis.</p>
        ...
        <div className="w-full flex gap-3">
          ...
          <button
            ...
            disabled={loading}
            className="flex-1 border border-white/20 text-white py-3 rounded-xl hover:bg-white/10 disabled:opacity-50"
          >
            Clear
          </button>
        </div>
